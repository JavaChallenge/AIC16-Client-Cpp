...
bool Network::doReceive() {
	try {
		...
		Packet.push_back(cache);
		for (int i = 0; i < sizeMessage; i++) {
			if (buf[i] != '\0') {
				Packet[Packet.size() - 1]->push(buf[i]);
			} else {
				parse();
				Packet.push_back(new SubPacket(MAX_LEN_OF_TCP));
			}
		}
		if (sizeMessage < 0)
			return false;
...
void Network::parse() {
	...
	for (int i = 0; i < (int) Packet.size(); i++) {
		str.append(Packet[i]->buffer.begin(),
				Packet[i]->buffer.begin() + Packet[i]->index);
	}
